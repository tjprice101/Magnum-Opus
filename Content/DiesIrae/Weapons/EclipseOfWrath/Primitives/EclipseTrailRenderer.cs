...
namespace MagnumOpus.Content.DiesIrae.Weapons.EclipseOfWrath.Primitives
{
    ...
    public sealed class EclipseTrailRenderer : ModSystem
    {
        ...
        {
            vertices = new EclipseVertexType[MaxVertices];
            indices = new short[MaxIndices];
            Main.QueueMainThreadAction(() =>
            {
                var device = Main.graphics.GraphicsDevice;
                vertexBuffer = new DynamicVertexBuffer(device, EclipseVertexType.VertexDecl, MaxVertices, BufferUsage.WriteOnly);
        ...
        public static void RenderTrail(IList<Vector2> controlPoints, EclipseTrailSettings settings)
        {
            ...
            for (int i = 0; i < segCount; i++)
            {
                ...
                else if (i == segCount - 1) tangent = smoothed[segCount - 1] - smoothed[segCount - 2];
                ...
                vertices[i * 2] = new EclipseVertexType(screenPos + normal * width * 0.5f, color, new Vector3(progress, 0f, width));
                vertices[i * 2 + 1] = new EclipseVertexType(screenPos - normal * width * 0.5f, color, new Vector3(progress, 1f, width));
            }
        ...
        private static List<Vector2> SmoothPoints(IList<Vector2> points, int steps)
        {
            ...
            for (int i = 0; i < points.Count - 1; i++)
            {
                ...
                for (int s = 0; s < steps; s++)
                    result.Add(Vector2.CatmullRom(p0, p1, p2, p3, (float)s / steps));
            }
