...
{
    /// <summary>
    /// Abstract base particle for Wrathful Contract VFX.
    /// </summary>
    public abstract class DemonParticle
    {
    ...
    /// <summary>
    /// Particle handler for Wrathful Contract — self-contained per-weapon particle manager.
    /// </summary>
    public class DemonParticleHandler : ModSystem
    {
        private const int MaxParticles = 400;
        private static readonly List<DemonParticle> _particles = new();
        ...
        public static void Spawn(DemonParticle p)
        {
        ...
        private static void DrawParticles(On_Main.orig_DrawDust orig, Main self)
        {
            ...
            for (int i = _particles.Count - 1; i >= 0; i--)
            {
                var p = _particles[i];
                p.Update();
                if (!p.Active) { _particles.RemoveAt(i); continue; }
