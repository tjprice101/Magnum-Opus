...
{
    /// <summary>
    /// Self-contained particle handler for Chain of Judgment. 400-cap. Two-pass rendering.
    /// </summary>
    public class ChainParticleHandler : ModSystem
    {
        private static readonly List<ChainParticle> particles = new();
        private const int MaxParticles = 400;

        public static void Spawn(ChainParticle p)
        {
        ...
        private static void DrawParticles(On_Main.orig_DrawDust orig, Main self)
        {
            ...
            foreach (var p in particles)
                if (p.Active && !p.IsAdditive) p.Draw(sb);
            sb.End();
