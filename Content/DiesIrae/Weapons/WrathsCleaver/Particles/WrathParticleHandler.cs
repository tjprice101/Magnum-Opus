...
{
    /// <summary>
    /// Self-contained particle handler for Wrath's Cleaver.
    /// Manages its own particle list, update loop, and draw passes.
    /// 600-particle cap, two-pass rendering (additive + alpha blend).
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class WrathParticleHandler : ModSystem
    {
        private static List<WrathParticle> activeParticles;
        private static List<WrathParticle> particlesToRemove;
        private const int ParticleLimit = 600;

        public override void Load()
        {
            activeParticles = new List<WrathParticle>(ParticleLimit);
            particlesToRemove = new List<WrathParticle>();
        ...
        public static void SpawnParticle(WrathParticle particle)
        {
