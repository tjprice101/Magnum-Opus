...
{
    /// <summary>
    /// Self-contained particle handler for Executioner's Verdict.
    /// 500-particle cap, two-pass rendering (additive + alpha blend).
    /// Hooked into On_Main.DrawDust for consistent draw order.
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class VerdictParticleHandler : ModSystem
    {
        private static List<VerdictParticle> activeParticles;
        private static List<VerdictParticle> particlesToRemove;
        private const int ParticleLimit = 500;

        public override void Load()
        {
            activeParticles = new List<VerdictParticle>(ParticleLimit);
            particlesToRemove = new List<VerdictParticle>();
        ...
        public static void SpawnParticle(VerdictParticle particle)
        {
        ...
        private static void DrawAllParticles(On_Main.orig_DrawDust orig, Main self)
        {
            ...
            // Pass 1: Additive blend (glows, blooms, execution marks)
            ...
            // Pass 2: Alpha blend (smoke, blood drips, debris)
