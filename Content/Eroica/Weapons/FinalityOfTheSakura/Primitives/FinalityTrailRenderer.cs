...
namespace MagnumOpus.Content.Eroica.Weapons.FinalityOfTheSakura.Primitives
{
    public static class FinalityTrailRenderer
    {
        ...
        public static void RenderTrail(Vector2[] positions, FinalityTrailSettings settings, int maxPointCount = 40)
        {
            ...
            if (_vertexBuffer == null || _vertexBuffer.IsDisposed)
                _vertexBuffer = new DynamicVertexBuffer(device, FinalityVertexType._vertexDeclaration, MaxVertices, BufferUsage.WriteOnly);
            ...
            FinalityVertexType[] vertices = new FinalityVertexType[vertexCount];
            ...
            for (int i = 0; i < pointCount; i++)
            {
                ...
                vertices[i * 2] = new FinalityVertexType(worldPos + normal * width * 0.5f, color, new Vector3(completion, 0f, width));
                vertices[i * 2 + 1] = new FinalityVertexType(worldPos - normal * width * 0.5f, color, new Vector3(completion, 1f, width));
            }
            ...
            // FIXED shader pattern — direct Apply(), no GetVertexShader()
