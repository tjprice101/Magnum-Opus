...
{
    /// <summary>
    /// Immutable configuration for a Black Swan primitive trail.
    /// Delegates define width, color, and offset per-vertex.
    /// </summary>
    public readonly struct BlackSwanTrailSettings
    {
    ...
    /// <summary>
    /// Self-contained GPU primitive trail renderer for Call of the Black Swan.
    /// Builds a triangle-strip ribbon mesh from position arrays and renders with shaders.
    /// </summary>
    public class BlackSwanPrimitiveRenderer : IDisposable
    {
        ...
        private BlackSwanVertex[] _vertices;
        private short[] _indices;
        private bool _disposed;

        public BlackSwanPrimitiveRenderer()
        {
            _vertices = new BlackSwanVertex[MaxVertices];
        ...
        private void EnsureBuffers(GraphicsDevice device)
        {
            if (_vertexBuffer == null || _vertexBuffer.IsDisposed)
                _vertexBuffer = new DynamicVertexBuffer(device, BlackSwanVertex.Declaration, MaxVertices, BufferUsage.WriteOnly);
        ...
        /// <summary>
        /// Render a trail from world-space positions using the given settings.
        /// </summary>
        public void RenderTrail(Vector2[] positions, BlackSwanTrailSettings settings, int? renderPoints = null)
        {
            if (positions == null || positions.Length < 2)
                return;

            // Filter out zero/invalid positions
            ...
            // Resample to desired point count if needed
            ...
            // Compute completion ratios based on arc length
            float[] completionRatios = ComputeCompletionRatios(sampledPositions);

            // Build the triangle strip mesh
            int vertexCount = 0;
            int indexCount = 0;
            ...
            for (int i = 0; i < sampledPositions.Length; i++)
            {
                ...
                // Compute normal (perpendicular to the trail direction)
                Vector2 tangent;
                if (i == 0)
                ...
                // Texture coordinates: U along trail length, V across width
                float u = t;
                float widthCorrection = Math.Max(halfWidth, 0.01f);

                int vi = vertexCount;
                _vertices[vi] = new BlackSwanVertex(left, color, new Vector3(u, 0f, widthCorrection));
                _vertices[vi + 1] = new BlackSwanVertex(right, color, new Vector3(u, 1f, widthCorrection));
                vertexCount += 2;
        ...
        {
            var result = new System.Collections.Generic.List<Vector2>(positions.Length);
        ...
        /// <summary>
        /// Catmull-Rom smoothing of positions for nicer curves.
        /// </summary>
