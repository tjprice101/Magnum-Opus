...
namespace MagnumOpus.Content.SwanLake.ResonantWeapons.IridescentWingspan.Primitives
{
    public readonly struct WingspanVertex : IVertexType
    {
        ...
        public WingspanVertex(Vector2 pos, Color col, Vector3 tc)
        { Position = pos; Color = col; TextureCoordinates = tc; }
    }

    public readonly struct WingspanTrailSettings
    {
        ...
        public WingspanTrailSettings(WidthFunc w, ColorFunc c, Terraria.Graphics.Shaders.MiscShaderData s = null)
        { Width = w; TrailColor = c; Shader = s; }
    }

    public class WingspanPrimitiveRenderer : IDisposable
    {
        private const int MaxV = 2048, MaxI = 6144;
        private DynamicVertexBuffer _vb; private DynamicIndexBuffer _ib;
        private WingspanVertex[] _v = new WingspanVertex[MaxV];
        private short[] _ix = new short[MaxI];
        private bool _disposed;

        private void Ensure(GraphicsDevice gd)
        {
            if (_vb == null || _vb.IsDisposed) _vb = new DynamicVertexBuffer(gd, WingspanVertex.Declaration, MaxV, BufferUsage.WriteOnly);
            if (_ib == null || _ib.IsDisposed) _ib = new DynamicIndexBuffer(gd, IndexElementSize.SixteenBits, MaxI, BufferUsage.WriteOnly);
        }

        public void RenderTrail(Vector2[] positions, WingspanTrailSettings settings, int? pts = null)
        {
            ...
            float td = 0; for (int i = 1; i < arr.Length; i++) td += Vector2.Distance(arr[i-1], arr[i]);
            int vc = 0, ic = 0; float acc = 0;
            for (int i = 0; i < arr.Length; i++)
            {
                float t = td > 0.001f ? acc / td : (float)i / (arr.Length - 1);
                ...
                if (tan.LengthSquared() < 0.0001f) tan = Vector2.UnitX; tan.Normalize();
                Vector2 n = new(-tan.Y, tan.X); Vector2 sp = arr[i]-Main.screenPosition;
                _v[vc] = new(sp+n*hw, c, new Vector3(t,0,hw));
                _v[vc+1] = new(sp-n*hw, c, new Vector3(t,1,hw));
                if (i > 0) { int pv=vc-2; _ix[ic++]=(short)pv; _ix[ic++]=(short)(pv+1); _ix[ic++]=(short)vc; _ix[ic++]=(short)(pv+1); _ix[ic++]=(short)(vc+1); _ix[ic++]=(short)vc; }
                vc+=2; if (vc>=MaxV-2||ic>=MaxI-6) break;
            }
            if (vc < 4 || ic < 6) return;
            var dev = Main.graphics.GraphicsDevice; Ensure(dev);
            ...
            _vb.SetData(_v,0,vc,SetDataOptions.Discard); _ib.SetData(_ix,0,ic,SetDataOptions.Discard);
        ...
        static Vector2[] Resample(Vector2[] s, int n)
        {
            if (s.Length==n) return s; var r=new Vector2[n];
