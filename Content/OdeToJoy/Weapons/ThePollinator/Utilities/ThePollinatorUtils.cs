...
namespace MagnumOpus.Content.OdeToJoy.Weapons.ThePollinator.Utilities
{
    ...
    /// Palette: ranged pollination — dark green through leaf green and pollen gold to bloom gold warmth.
    /// </summary>
    public static class ThePollinatorUtils
    {
        #region Color Palette — Pollination

        /// <summary>Core 6-color palette: dark green → leaf green → pollen gold → bloom gold → cream → white.</summary>
        public static readonly Color[] WeaponPalette = new Color[]
        {
            OdeToJoyPalette.DeepForest,        // Dark green (20, 60, 15)
            OdeToJoyPalette.LeafGreen,         // Leaf green (50, 140, 40)
        ...
        /// <summary>Get a cycling pollen hue — green-gold transition like sunlit meadow.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.18f + hue * 0.14f; // Green-gold hue range 0.18-0.32
            return Main.hslToRgb(hue, 0.80f, 0.58f);
        }
