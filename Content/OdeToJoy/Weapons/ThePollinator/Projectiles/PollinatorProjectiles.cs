...
namespace MagnumOpus.Content.OdeToJoy.Weapons.ThePollinator.Projectiles
{
    ...
    public class PollenShotProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            // SmokeFoundation-style pollen cloud trail — custom pollen dust
            if (Main.rand.NextBool(2))
            {
                Dust d = Dust.NewDustDirect(Projectile.position - new Vector2(4), Projectile.width + 8, Projectile.height + 8, ModContent.DustType<PollenCloudDust>(), -Projectile.velocity.X * 0.1f, -Projectile.velocity.Y * 0.1f, 150, default, 0.8f);
                d.noGravity = true;
                d.fadeIn = 1.2f;
                d.velocity *= 0.5f;
            }
            // Blossom accent sparkles — pollen motes
            if (Main.rand.NextBool(6))
            {
                Dust d = Dust.NewDustDirect(Projectile.Center - new Vector2(2), 4, 4, ModContent.DustType<PollenCloudDust>(), 0f, 0f, 100, default, 0.5f);
                d.noGravity = true;
            }
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            // Pollen burst on impact — custom pollen cloud
            for (int i = 0; i < 12; i++)
            {
                Vector2 vel = Main.rand.NextVector2Circular(3f, 3f);
                Dust d = Dust.NewDustDirect(target.Center - new Vector2(4), 8, 8, ModContent.DustType<PollenCloudDust>(), vel.X, vel.Y, 120, default, 1.0f);
                d.noGravity = true;
                d.fadeIn = 1.5f;
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            try
            {
                ...
                // Pollen shot: golden pollen cloud haze
                sb.End();
                ...
                if (glow != null)
                {
                    ...
                    float pulse = 0.8f + 0.2f * MathF.Sin((float)Main.timeForVisualEffects * 0.12f + Projectile.whoAmI);
    ...
    public class MassBloomProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (_timer == 1)
            {
                ...
                // 55 radial pollen/petal burst particles — custom pollen dust
                for (int i = 0; i < 55; i++)
                {
                    float angle = MathHelper.TwoPi * i / 55f;
                    float speed = 3f + Main.rand.NextFloat() * 5f;
                    Vector2 vel = new Vector2((float)Math.Cos(angle), (float)Math.Sin(angle)) * speed;
                    Dust d = Dust.NewDustDirect(Projectile.Center, 1, 1, ModContent.DustType<PollenCloudDust>(), vel.X, vel.Y, 100, default, 1.1f);
                    d.noGravity = true;
                    d.fadeIn = 1.6f;
                }

                // Mass Bloom screen effects
                OdeToJoyVFXLibrary.ScreenShake(8f, 16);
                OdeToJoyVFXLibrary.ScreenFlash(OdeToJoyPalette.GoldenPollen, 1.2f);
                OdeToJoyVFXLibrary.CelebrationBurst(Projectile.Center, 1.3f);
            }
        }
    ...
    public class HomingSeedProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            // Golden sparkle trail — custom pollen motes
            if (Main.rand.NextBool(2))
            {
                Dust d = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<PollenCloudDust>(), 0f, 0f, 130, default, 0.5f);
                d.noGravity = true;
                d.velocity *= 0.2f;
            }
        }
        ...
        {
            target.AddBuff(ModContent.BuffType<PollinatedDebuff>(), 600);
            OdeToJoyVFXLibrary.SpawnGardenSparkleExplosion(target.Center, 2, 2f, 0.4f);
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            try
            {
                ...
                // Homing seed: verdant directional streak
                sb.End();
                sb.Begin(SpriteSortMode.Deferred, MagnumBlendStates.TrueAdditive,
                    SamplerState.LinearClamp, DepthStencilState.None,
                    RasterizerState.CullNone, null, Main.GameViewMatrix.TransformationMatrix);
                ...
                if (glow != null)
                {
                    ...
                    sb.Draw(glow, drawPos, null,
                        (OdeToJoyPalette.VerdantGreen with { A = 0 }) * 0.18f,
    ...
    public class GoldenFieldProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            // Ambient pollen particles — custom pollen cloud
            if (Main.rand.NextBool(4))
            {
                ...
                Dust d = Dust.NewDustDirect(pos, 1, 1, ModContent.DustType<PollenCloudDust>(), 0f, -0.5f, 150, default, 0.5f);
                d.noGravity = true;
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
            Texture2D glowTex = PollinatorTextures.SoftGlow;
            Texture2D maskTex = PollinatorTextures.SoftCircle;
            Vector2 glowOrigin = glowTex.Size() / 2f;
