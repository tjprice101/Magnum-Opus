...
namespace MagnumOpus.Content.OdeToJoy.Weapons.RoseThornChainsaw.Utilities
{
    ...
    /// Palette: melee chainsaw — dark rose through rose red and thorn green to bloom gold fury.
    /// </summary>
    public static class RoseThornChainsawUtils
    {
        #region Color Palette — Rose & Thorn Chainsaw

        /// <summary>Core 6-color palette: dark rose → rose red → thorn green → bloom gold → petal pink → white.</summary>
        ...
        /// <summary>Get a cycling rose-to-thorn hue — warm pink shifting toward green edge.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.92f + hue * 0.14f; // Rose-pink hue range 0.92-1.06 (wraps)
            if (hue > 1f) hue -= 1f;
            return Main.hslToRgb(hue, 0.80f, 0.62f);
        }
