...
namespace MagnumOpus.Content.OdeToJoy.Weapons.ThornboundReckoning.Utilities
{
    ...
    /// Palette: melee thorn-themed — dark earth through thorn green and rose red to bloom gold brilliance.
    /// </summary>
    public static class ThornboundReckoningUtils
    {
        #region Color Palette — Thorn & Rose

        /// <summary>Core 6-color palette: dark earth → thorn green → rose red → bloom gold → petal white → radiant white.</summary>
        ...
        /// <summary>Get a cycling thorn-green to rose hue — earthy green range with rose warmth.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.25f + hue * 0.12f; // Green-rose hue range 0.25-0.37
            return Main.hslToRgb(hue, 0.85f, 0.65f);
        }
