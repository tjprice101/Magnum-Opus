...
namespace MagnumOpus.Content.OdeToJoy.Weapons.ElysianVerdict.Utilities
{
    ...
    /// Palette: melee golden justice — dark amber through sunbeam gold to pure white radiance.
    /// </summary>
    public static class ElysianVerdictUtils
    {
        #region Color Palette — Golden Justice

        /// <summary>Core 6-color palette: dark amber → amber → sunbeam gold → radiant gold → cream white → pure white.</summary>
        public static readonly Color[] WeaponPalette = new Color[]
        {
            new Color(120, 80, 15),            // Dark amber
            ...
            OdeToJoyPalette.WhiteBloom,        // Cream white (255, 250, 235)
        ...
        /// <summary>Get a cycling golden justice hue — warm amber to sunbeam gold range.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.08f + hue * 0.10f; // Gold hue range 0.08-0.18
            return Main.hslToRgb(hue, 0.90f, 0.65f);
        }
