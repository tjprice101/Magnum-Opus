...
{
    /// <summary>
    /// Self-contained utility library for Fountain of Joyous Harmony.
    ...
    /// Palette: magic fountain — deep blue tint through water teal and bloom gold to petal pink luminance.
    /// </summary>
    public static class FountainOfJoyousHarmonyUtils
    {
        #region Color Palette — Joyous Fountain

        /// <summary>Core 6-color palette: deep blue tint → water teal → bloom gold → petal pink → cream → white.</summary>
        public static readonly Color[] WeaponPalette = new Color[]
        {
            new Color(30, 50, 90),             // Deep blue tint
            new Color(60, 180, 170),           // Water teal
        ...
        /// <summary>Get a cycling fountain hue — teal-blue shifting through aqua-gold shimmer.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.45f + hue * 0.12f; // Teal-aqua hue range 0.45-0.57
            return Main.hslToRgb(hue, 0.78f, 0.60f);
        }
