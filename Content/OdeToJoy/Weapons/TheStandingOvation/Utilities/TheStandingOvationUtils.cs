...
{
    /// <summary>
    /// Self-contained utility library for The Standing Ovation.
    ...
    /// Palette: summoner ovation — deep gold through amber and sunbeam gold to radiant golden white.
    /// </summary>
    public static class TheStandingOvationUtils
    {
        #region Color Palette — Standing Ovation

        /// <summary>Core 6-color palette: deep gold → amber → sunbeam gold → radiant gold → golden white → pure white.</summary>
        public static readonly Color[] WeaponPalette = new Color[]
        {
            new Color(100, 75, 10),            // Deep gold
            OdeToJoyPalette.WarmAmber,         // Amber (200, 150, 40)
            OdeToJoyPalette.GoldenPollen,      // Sunbeam gold (255, 210, 60)
            OdeToJoyPalette.SunlightYellow,    // Radiant gold (255, 240, 120)
            OdeToJoyPalette.WhiteBloom,        // Golden white (255, 250, 235)
        ...
        /// <summary>Get a cycling ovation gold hue — rich amber through celebratory sunbeam.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.09f + hue * 0.09f; // Amber-gold hue range 0.09-0.18
            return Main.hslToRgb(hue, 0.88f, 0.66f);
        }
