...
{
    /// <summary>
    /// Self-contained utility library for Triumphant Chorus.
    ...
    /// Palette: summoner triumphant — deep amber through golden amber and celebration gold to radiant white.
    /// </summary>
    public static class TriumphantChorusUtils
    {
        #region Color Palette — Triumphant Chorus

        /// <summary>Core 6-color palette: deep amber → golden amber → celebration gold → joyful gold → warm white → radiant white.</summary>
        public static readonly Color[] WeaponPalette = new Color[]
        {
            new Color(120, 80, 15),            // Deep amber
            OdeToJoyPalette.WarmAmber,         // Golden amber (200, 150, 40)
            OdeToJoyPalette.GoldenPollen,      // Celebration gold (255, 210, 60)
            OdeToJoyPalette.SunlightYellow,    // Joyful gold (255, 240, 120)
            OdeToJoyPalette.WhiteBloom,        // Warm white (255, 250, 235)
            new Color(255, 255, 255),          // Radiant white
        };

        /// <summary>Lore tooltip color — Ode to Joy warm gold.</summary>
        public static readonly Color LoreColor = new Color(255, 200, 50);

        /// <summary>Get a cycling triumphant hue — celebratory gold with warm amber undertones.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.07f + hue * 0.11f; // Triumphant gold hue range 0.07-0.18
            return Main.hslToRgb(hue, 0.90f, 0.64f);
        }
