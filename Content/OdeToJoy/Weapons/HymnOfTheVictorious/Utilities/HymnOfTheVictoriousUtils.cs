...
namespace MagnumOpus.Content.OdeToJoy.Weapons.HymnOfTheVictorious.Utilities
{
    ...
    /// Palette: melee hymnal — deep gold shadow through amber and golden light to pure white glory.
    /// </summary>
    public static class HymnOfTheVictoriousUtils
    {
        #region Color Palette — Hymnal Gold

        /// <summary>Core 6-color palette: deep gold shadow → amber → golden light → joyful yellow → warm white → pure white.</summary>
        public static readonly Color[] WeaponPalette = new Color[]
        {
            new Color(80, 60, 10),             // Deep gold shadow
            OdeToJoyPalette.WarmAmber,         // Amber (200, 150, 40)
            OdeToJoyPalette.GoldenPollen,      // Golden light (255, 210, 60)
            OdeToJoyPalette.SunlightYellow,    // Joyful yellow (255, 240, 120)
        ...
        /// <summary>Get a cycling hymnal gold hue — warm amber sweeping through golden light.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.10f + hue * 0.08f; // Warm gold hue range 0.10-0.18
            return Main.hslToRgb(hue, 0.88f, 0.68f);
        }
