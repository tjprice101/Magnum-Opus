...
{
    /// <summary>
    /// Self-contained utility library for Thorn Spray Repeater.
    ...
    /// Palette: ranged thorns — dark green through thorn green and leaf green to bloom gold brilliance.
    /// </summary>
    public static class ThornSprayRepeaterUtils
    {
        #region Color Palette — Thorn Spray

        /// <summary>Core 6-color palette: dark green → thorn green → leaf green → bloom gold → petal white → white.</summary>
        public static readonly Color[] WeaponPalette = new Color[]
        {
            OdeToJoyPalette.DeepForest,        // Dark green (20, 60, 15)
            OdeToJoyPalette.LeafGreen,         // Thorn green (50, 140, 40)
            OdeToJoyPalette.BudGreen,          // Leaf green (70, 170, 50)
            OdeToJoyPalette.GoldenPollen,      // Bloom gold (255, 210, 60)
            OdeToJoyPalette.WhiteBloom,        // Petal white (255, 250, 235)
        ...
        /// <summary>Get a cycling thorn-green hue — verdant green range with golden highlights.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.26f + hue * 0.10f; // Green hue range 0.26-0.36
            return Main.hslToRgb(hue, 0.82f, 0.55f);
        }
