...
namespace MagnumOpus.Content.OdeToJoy.Weapons.AnthemOfGlory.Utilities
{
    ...
    /// Palette: ranged glory anthem — deep amber through sunbeam gold to pure white radiance.
    /// </summary>
    public static class AnthemOfGloryUtils
    {
        #region Color Palette — Glory Anthem

        /// <summary>Core 6-color palette: deep amber → amber → sunbeam gold → radiant gold → golden white → pure white.</summary>
        ...
        /// <summary>Get a cycling glory-gold hue — radiant amber through bright sunbeam.</summary>
        public static Color GetJoyCycle(float offset = 0f)
        {
            float hue = (Main.GameUpdateCount * 0.015f + offset) % 1f;
            hue = 0.08f + hue * 0.10f; // Gold-amber hue range 0.08-0.18
            return Main.hslToRgb(hue, 0.92f, 0.62f);
        }
