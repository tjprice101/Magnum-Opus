...
namespace MagnumOpus.Content.OdeToJoy.Weapons.SinCollector.Utilities
{
    /// <summary>
    /// Self-contained utility library for Sin Collector.
    /// Easing functions, piecewise animation, color helpers, and SpriteBatch extensions.
    /// Palette: sin collection — moss shadow through deep forest to golden sunlight.
    /// </summary>
    public static class SinCollectorUtils
    {
        ...
        {
            OdeToJoyPalette.MossShadow,
            OdeToJoyPalette.DeepForest,
            OdeToJoyPalette.LeafGreen,
            OdeToJoyPalette.GoldenPollen,
            OdeToJoyPalette.SunlightYellow,
            OdeToJoyPalette.WhiteBloom,
        };
        ...
        {
            increment %= 0.999f;
            int index = (int)(increment * colors.Length);
            float segmentProgress = increment * colors.Length % 1f;
            return Color.Lerp(colors[index], colors[(index + 1) % colors.Length], segmentProgress);
        }
