...
{
    /// <summary>
    /// GPU primitive trail renderer for FugueOfTheUnknown.
    /// Builds and renders a triangle-strip from a list of positions using FugueVertex.
    /// Supports CatmullRom spline smoothing and arc-length parameterization.
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class FuguePrimitiveRenderer : ModSystem
    {
        private static DynamicVertexBuffer _vertexBuffer;
        private static DynamicIndexBuffer _indexBuffer;
        private static FugueVertex[] _vertices;
        ...
        public override void OnModLoad()
        {
            Main.QueueMainThreadAction(() =>
            {
                ...
                _vertexBuffer = new DynamicVertexBuffer(device, FugueVertex.VertexDecl, MaxVertices, BufferUsage.WriteOnly);
                _indexBuffer = new DynamicIndexBuffer(device, IndexElementSize.SixteenBits, MaxIndices, BufferUsage.WriteOnly);
                _vertices = new FugueVertex[MaxVertices];
        ...
        /// </summary>
        public static void RenderTrail(List<Vector2> positions, FuguePrimitiveSettings settings)
        {
            ...
            // Build vertices (triangle list via indexed quads)
            int vertCount = 0;
            int idxCount = 0;
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                float u = completion;
                _vertices[vertCount] = new FugueVertex(screenPos + normal * width * 0.5f, color, new Vector2(u, 0f), widthCorrection);
                _vertices[vertCount + 1] = new FugueVertex(screenPos - normal * width * 0.5f, color, new Vector2(u, 1f), widthCorrection);
