...
{
    /// <summary>
    /// ModSystem that manages FugueOfTheUnknown-exclusive particles.
    /// Hooks into Main.DrawDust for dual-pass rendering (additive + alpha).
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class FugueParticleHandler : ModSystem
    {
        private static readonly List<FugueParticle> _particles = new(500);
        private const int MaxParticles = 500;

        public override void OnModLoad()
        {
            On_Main.DrawDust += DrawFugueParticles;
        }
        ...
        public static void Spawn(FugueParticle particle)
        {
        ...
        private void DrawFugueParticles(On_Main.orig_DrawDust orig, Main self)
        {
            orig(self);
