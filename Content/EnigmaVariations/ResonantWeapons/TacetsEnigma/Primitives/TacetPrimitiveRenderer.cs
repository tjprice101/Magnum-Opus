...
namespace MagnumOpus.Content.EnigmaVariations.ResonantWeapons.TacetsEnigma.Primitives
{
    /// <summary>
    /// GPU primitive trail renderer for TacetsEnigma's bullet and paradox bolt trails.
    /// Builds and renders a triangle-list from a list of positions using TacetVertex.
    /// Supports CatmullRom spline smoothing and arc-length parameterization.
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class TacetPrimitiveRenderer : ModSystem
    {
        private static DynamicVertexBuffer _vertexBuffer;
        private static DynamicIndexBuffer _indexBuffer;
        private static TacetVertex[] _vertices;
        ...
        public override void OnModLoad()
        {
            Main.QueueMainThreadAction(() =>
            {
                ...
                _vertexBuffer = new DynamicVertexBuffer(device, TacetVertex.VertexDecl, MaxVertices, BufferUsage.WriteOnly);
                _indexBuffer = new DynamicIndexBuffer(device, IndexElementSize.SixteenBits, MaxIndices, BufferUsage.WriteOnly);
                _vertices = new TacetVertex[MaxVertices];
        ...
        /// </summary>
        public static void RenderTrail(List<Vector2> positions, TacetPrimitiveSettings settings)
        {
            ...
            // Build vertices (triangle list via strip pairs)
            int vertCount = 0;
            int idxCount = 0;
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                float u = completion;
                _vertices[vertCount] = new TacetVertex(screenPos + normal * width * 0.5f, color, new Vector2(u, 0f), widthCorrection);
                _vertices[vertCount + 1] = new TacetVertex(screenPos - normal * width * 0.5f, color, new Vector2(u, 1f), widthCorrection);
