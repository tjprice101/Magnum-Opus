...
{
    /// <summary>
    /// ModSystem that manages TacetsEnigma-exclusive particles.
    /// Hooks into Main.DrawDust for dual-pass rendering (additive + alpha).
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class TacetParticleHandler : ModSystem
    {
        private static readonly List<TacetParticle> _particles = new(500);
        private const int MaxParticles = 500;

        public override void OnModLoad()
        {
            On_Main.DrawDust += DrawTacetParticles;
        }
        ...
        public static void Spawn(TacetParticle particle)
        {
        ...
        private void DrawTacetParticles(On_Main.orig_DrawDust orig, Main self)
        {
            orig(self);
