...
{
    /// <summary>
    /// ModSystem that manages CipherNocturne-exclusive particles.
    /// Hooks into Main.DrawDust for dual-pass rendering (additive + alpha).
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class CipherParticleHandler : ModSystem
    {
        private static readonly List<CipherParticle> _particles = new(500);
        private const int MaxParticles = 500;

        public override void OnModLoad()
        {
            On_Main.DrawDust += DrawCipherParticles;
        }
        ...
        public static void Spawn(CipherParticle particle)
        {
        ...
        private void DrawCipherParticles(On_Main.orig_DrawDust orig, Main self)
        {
            orig(self);
