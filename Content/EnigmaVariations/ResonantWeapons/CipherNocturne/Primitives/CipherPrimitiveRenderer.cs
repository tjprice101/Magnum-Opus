...
namespace MagnumOpus.Content.EnigmaVariations.ResonantWeapons.CipherNocturne.Primitives
{
    /// <summary>
    /// GPU primitive trail renderer for CipherNocturne's beam strip.
    /// Builds and renders a triangle-strip from a list of positions using CipherVertex.
    /// Supports CatmullRom spline smoothing and arc-length parameterization.
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class CipherPrimitiveRenderer : ModSystem
    {
        private static DynamicVertexBuffer _vertexBuffer;
        private static DynamicIndexBuffer _indexBuffer;
        private static CipherVertex[] _vertices;
        ...
        public override void OnModLoad()
        {
            Main.QueueMainThreadAction(() =>
            {
                ...
                _vertexBuffer = new DynamicVertexBuffer(device, CipherVertex.VertexDecl, MaxVertices, BufferUsage.WriteOnly);
                _indexBuffer = new DynamicIndexBuffer(device, IndexElementSize.SixteenBits, MaxIndices, BufferUsage.WriteOnly);
                _vertices = new CipherVertex[MaxVertices];
        ...
        /// </summary>
        public static void RenderTrail(List<Vector2> positions, CipherPrimitiveSettings settings)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                float u = completion;
                _vertices[vertCount] = new CipherVertex(screenPos + normal * width * 0.5f, color, new Vector2(u, 0f), widthCorrection);
                _vertices[vertCount + 1] = new CipherVertex(screenPos - normal * width * 0.5f, color, new Vector2(u, 1f), widthCorrection);
