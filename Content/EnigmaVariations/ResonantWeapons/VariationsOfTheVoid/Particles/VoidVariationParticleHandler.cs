...
{
    /// <summary>
    /// ModSystem that manages VariationsOfTheVoid-exclusive particles.
    /// Hooks into Main.DrawDust for dual-pass rendering (additive + alpha).
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class VoidVariationParticleHandler : ModSystem
    {
        private static readonly List<VoidVariationParticle> _particles = new(500);
        private const int MaxParticles = 500;

        public override void OnModLoad()
        {
            On_Main.DrawDust += DrawVoidVariationParticles;
        }
        ...
        public static void Spawn(VoidVariationParticle particle)
        {
        ...
        private void DrawVoidVariationParticles(On_Main.orig_DrawDust orig, Main self)
        {
            orig(self);
