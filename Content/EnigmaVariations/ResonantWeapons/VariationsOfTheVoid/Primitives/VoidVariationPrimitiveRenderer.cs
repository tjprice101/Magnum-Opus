...
{
    /// <summary>
    /// GPU primitive trail renderer for VariationsOfTheVoid.
    /// Builds and renders a triangle-list from a list of positions using VoidVariationVertex.
    /// Supports CatmullRom spline smoothing and arc-length parameterization.
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class VoidVariationPrimitiveRenderer : ModSystem
    {
        private static DynamicVertexBuffer _vertexBuffer;
        private static DynamicIndexBuffer _indexBuffer;
        private static VoidVariationVertex[] _vertices;
        ...
        public override void OnModLoad()
        {
            Main.QueueMainThreadAction(() =>
            {
                ...
                _vertexBuffer = new DynamicVertexBuffer(device, VoidVariationVertex.VertexDecl, MaxVertices, BufferUsage.WriteOnly);
                _indexBuffer = new DynamicIndexBuffer(device, IndexElementSize.SixteenBits, MaxIndices, BufferUsage.WriteOnly);
                _vertices = new VoidVariationVertex[MaxVertices];
        ...
        /// </summary>
        public static void RenderTrail(List<Vector2> positions, VoidVariationPrimitiveSettings settings)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                float u = completion;
                _vertices[vertCount] = new VoidVariationVertex(screenPos + normal * width * 0.5f, color, new Vector2(u, 0f), widthCorrection);
                _vertices[vertCount + 1] = new VoidVariationVertex(screenPos - normal * width * 0.5f, color, new Vector2(u, 1f), widthCorrection);
