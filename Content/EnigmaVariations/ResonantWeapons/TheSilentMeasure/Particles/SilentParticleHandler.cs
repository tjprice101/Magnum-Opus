...
{
    /// <summary>
    /// ModSystem that manages TheSilentMeasure-exclusive particles.
    /// Hooks into Main.DrawDust for dual-pass rendering (additive + alpha).
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class SilentParticleHandler : ModSystem
    {
        private static readonly List<SilentParticle> _particles = new(500);
        private const int MaxParticles = 500;

        public override void OnModLoad()
        {
            On_Main.DrawDust += DrawSilentParticles;
        }
        ...
        public static void Spawn(SilentParticle particle)
        {
        ...
        private void DrawSilentParticles(On_Main.orig_DrawDust orig, Main self)
        {
            orig(self);
