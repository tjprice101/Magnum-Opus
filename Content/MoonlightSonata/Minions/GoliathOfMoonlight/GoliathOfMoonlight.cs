...
{
    /// <summary>
    /// Goliath of Moonlight 驕ｯ・ｶ郢晢ｽｻA massive hovering lunar guardian minion.
    /// State machine AI: Idle 驕ｶ鄙ｫ繝ｻChase 驕ｶ鄙ｫ繝ｻBeamAttack 驕ｶ鄙ｫ繝ｻDevastatingBeam.
    ...
    public class GoliathOfMoonlight : ModProjectile
    {
        ...
        private void FireMoonlightBeam(Player owner)
        {
            ...
            // Fire VFX 驕ｯ・ｶ郢晢ｽｻcolored by lunar phase
            if (!Main.dedServ)
            {
        ...
        private void FireDevastatingBeam(Player owner)
        {
            ...
            // Dramatic fire VFX 驕ｯ・ｶ郢晢ｽｻphase tinted
            if (!Main.dedServ)
            {
                Color phaseColor = gp.CurrentPhaseColor;
                ...
                // Music note burst 驕ｯ・ｶ郢晢ｽｻ3 notes for devastating beam
                for (int i = 0; i < 3; i++)
                {
        ...
        private void SpawnAmbientVFX()
        {
            ...
            // Foundation VFX: GoliathMaskAura 窶・persistent aura overlay, refreshed every 100 ticks
            ...
            // Orbiting rift motes 驕ｯ・ｶ郢晢ｽｻevery 15 ticks, color tinted by lunar phase
            if (AliveTime % 15 == 0)
            {
            ...
            // Gravity well particles 驕ｯ・ｶ郢晢ｽｻevery 20 ticks
            if (AliveTime % 20 == 0)
            {
                GoliathParticleHandler.Spawn(new GravityWellParticle(
            ...
            // Cosmic dust 驕ｯ・ｶ郢晢ｽｻevery 8 ticks
            if (AliveTime % 8 == 0)
            {
            ...
            // Lunar phase ring 驕ｯ・ｶ郢晢ｽｻexpanding ring every 40 ticks showing current phase
            if (AliveTime % 40 == 0)
            {
                GoliathParticleHandler.Spawn(new LunarPhaseRingParticle(
            ...
            switch (lunarPhase)
            {
                case 0: // New Moon 驕ｯ・ｶ郢晢ｽｻsubtle void wisps
                    if (AliveTime % 25 == 0)
                    {
                        GoliathParticleHandler.Spawn(new GravityWellParticle(
                            Projectile.Center + Main.rand.NextVector2Circular(60f, 60f),
                ...
                case 1: // Waxing 驕ｯ・ｶ郢晢ｽｻgrowing energy sparks
                    if (AliveTime % 18 == 0)
                    {
                ...
                case 2: // Full Moon 驕ｯ・ｶ郢晢ｽｻradiant music notes + bright motes
                    if (AliveTime % 12 == 0)
                    {
                        GoliathParticleHandler.Spawn(new GoliathMusicNoteParticle(
                            Projectile.Center + Main.rand.NextVector2Circular(30f, 30f),
                ...
                case 3: // Waning 驕ｯ・ｶ郢晢ｽｻhealing orbs drifting toward player
                    if (AliveTime % 30 == 0)
                    {
            ...
            // Orbital music notes 驕ｯ・ｶ郢晢ｽｻevery 30 ticks (musical identity)
            if (AliveTime % 30 == 0)
            {
                GoliathParticleHandler.Spawn(new GoliathMusicNoteParticle(
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            // Pass 1: Gravitational rift shader glow (under the sprite) 窶・Additive
            sb.End();
            ...
            // Pass 2: Goliath sprite with 6x6 spritesheet 窶・AlphaBlend
            sb.End();
        ...
        private void DrawRiftGlow(SpriteBatch sb)
        {
            ...
            // Ambient rift glow 窶・static, tinted by lunar phase
            float pulse = 0.15f;
            ...
            // Reduced by 80% per user feedback 窶・Goliath bloom was obscuring the summon
            ...
            // Phase-specific accent glow 窶・reduced
        ...
        private void DrawGoliathGlow(SpriteBatch sb)
        {
            ...
            // Eye / core glow 窶・tinted by lunar phase (reduced 80% per feedback)
            ...
            // Full Moon phase: extra radiance 窶・reduced 80%
            if (gp.IsFullMoon)
            {
            ...
            // Conductor Mode: additional outer ring glow 窶・reduced 80%
            if (gp.ConductorMode)
            {
