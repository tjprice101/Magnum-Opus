...
{
    /// <summary>
    /// Self-contained GPU primitive trail renderer for the Incisor of Moonlight.
    /// Manages DynamicVertexBuffer/IndexBuffer and renders trail geometry with custom shaders.
    /// Completely independent of any shared mod primitive systems.
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class IncisorPrimitiveRenderer : ModSystem
    {
        private static DynamicVertexBuffer VertexBuffer;
        private static DynamicIndexBuffer IndexBuffer;
        private static IncisorPrimitiveSettings MainSettings;
        private static IncisorTopology ActiveTopology;
        private static Vector2[] MainPositions;
        private static Vector2[] MainTangents;
        private static Vector2[] MainNormals;
        private static IncisorVertex[] MainVertices;
        ...
        public override void OnModLoad()
        {
            Main.QueueMainThreadAction(() =>
            {
                ...
                MainVertices = new IncisorVertex[MaxVertices];
                ...
                VertexBuffer ??= new DynamicVertexBuffer(Main.instance.GraphicsDevice, IncisorVertex.VertexDecl, MaxVertices, BufferUsage.WriteOnly);
        ...
        public static void RenderTrail(List<Vector2> positions, IncisorPrimitiveSettings settings, int? pointsToCreate = null)
            => RenderTrail(positions.ToArray(), settings, pointsToCreate);

        public static void RenderTrail(Vector2[] positions, IncisorPrimitiveSettings settings, int? pointsToCreate = null)
        {
            ...
            ActiveTopology = settings.CapStyle != IncisorCapStyle.None ? IncisorTopology.TriangleList : settings.Topology;
        ...
        private static void PrivateRender()
        {
            ...
            if (ActiveTopology == IncisorTopology.TriangleList)
            {
                if (IndicesIndex < 6 || IndicesIndex % 3 != 0) return;
            }
            else if (ActiveTopology == IncisorTopology.TriangleStrip && IndicesIndex < 4) return;
            ...
            var shaderToUse = MainSettings.Shader ?? GameShaders.Misc["MagnumOpus:IncisorStandardPrimitive"];
            ...
            PrimitiveType primitiveType = ActiveTopology == IncisorTopology.TriangleStrip
        ...
        private static bool AssignPoints(Vector2[] positions, IncisorPrimitiveSettings settings, int pointsToCreate)
        {
        ...
        private static void AssignVertices()
        {
            ...
            for (int i = 0; i < PositionsIndex; i++)
            {
                ...
                MainVertices[VerticesIndex++] = new IncisorVertex(left, c, leftUV, effectiveHW);
                MainVertices[VerticesIndex++] = new IncisorVertex(right, c, rightUV, effectiveHW);
            }
        ...
        {
            if (MainSettings.CapStyle == IncisorCapStyle.None || PositionsIndex <= 0) return;
            if (ActiveTopology == IncisorTopology.TriangleStrip) return;
        ...
        private static short TryCreateCapVertex(int posIdx)
        {
            ...
            ref readonly IncisorVertex lv = ref MainVertices[leftVI];
            ref readonly IncisorVertex rv = ref MainVertices[rightVI];
            ...
            MainVertices[VerticesIndex++] = new IncisorVertex(centerPos, centerColor, centerTC, centerHW);
            return newVI;
        }
        ...
        private static float ComputeTextureCoord(int index, float completionRatio)
        {
            ...
            {
                IncisorTextureMode.Distance => (cr * TotalTrailLength + MainSettings.TextureScrollOffset) / cycleLen,
        ...
        private static void ComputeFrameData()
        {
            ...
            for (int i = 0; i < PositionsIndex; i++)
            {
                ...
                if (MainSettings.FrameTransportMode == IncisorFrameTransport.ParallelTransport && i > 0 && prevNormal.LengthSquared() > Epsilon)
                {
        ...
        private static void ComputeEdgePositions(int index, float halfWidth, out Vector2 left, out Vector2 right, out float effectiveHW)
        {
            ...
            if (MainSettings.JoinStyle == IncisorJoinStyle.Flat || PositionsIndex <= 2 || index == 0 || index == PositionsIndex - 1)
            {
        ...
        {
            IndicesIndex = 0;
            if (ActiveTopology == IncisorTopology.TriangleStrip)
            {
        ...
        private static Vector2 EvaluateCurve(IncisorSmoothingType type, Vector2 p0, Vector2 p1, Vector2 p2, Vector2 p3, float t, IncisorPrimitiveSettings settings)
        {
            t = MathHelper.Clamp(t, 0f, 1f);
            switch (type)
            {
                case IncisorSmoothingType.Linear:
                    return Vector2.Lerp(p1, p2, t);
                case IncisorSmoothingType.Cardinal:
                {
                ...
                case IncisorSmoothingType.Hermite:
                {
                    Vector2 m0 = 0.5f * (p2 - p0);
                    Vector2 m1 = 0.5f * (p3 - p1);
                    return EvalHermite(p1, p2, m0, m1, t);
                }
                case IncisorSmoothingType.CubicBezier:
                {
