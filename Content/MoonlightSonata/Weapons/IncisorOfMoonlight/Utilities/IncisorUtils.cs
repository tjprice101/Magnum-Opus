...
{
    /// <summary>
    /// Self-contained utility helpers for the Incisor of Moonlight.
    /// Easing functions, piecewise animation curves, color palette, and SpriteBatch extensions.
    /// </summary>
    public static class IncisorUtils
    {
        // =====================================================================
        // INCISOR PALETTE — Deep Resonance through Harmonic White
        // Musical dynamics: Pianissimo → Sforzando
        // =====================================================================
        public static readonly Color[] IncisorPalette = new Color[]
        {
        ...
        // =====================================================================
        // SPRITEBATCH EXTENSIONS — shader region enter/exit
        // =====================================================================
        ...
        {
            spriteBatch.End();
            spriteBatch.Begin(SpriteSortMode.Immediate, newBlendState ?? BlendState.AlphaBlend,
        ...
        public static NPC ClosestNPCAt(this Vector2 origin, float maxDist, bool ignoreTiles = true)
        {
            ...
            {
                if (!Main.npc[i].CanBeChasedBy()) continue;
                bool canHit = ignoreTiles || Collision.CanHit(origin, 1, 1, Main.npc[i].Center, 1, 1);
                float d = Vector2.Distance(origin, Main.npc[i].Center);
                if (d < distance && canHit)
                {
                    distance = d;
                    closest = Main.npc[i];
                }
        ...
        public static Vector2 RandomVelocity(float dirMult, float speedMin, float speedMax, float speedMult = 0.1f)
        {
            Vector2 v = new(Main.rand.NextFloat(-dirMult, dirMult), Main.rand.NextFloat(-dirMult, dirMult));
            while (v == Vector2.Zero)
                v = new(Main.rand.NextFloat(-dirMult, dirMult), Main.rand.NextFloat(-dirMult, dirMult));
            v.Normalize();
            v *= Main.rand.NextFloat(speedMin, speedMax) * speedMult;
            return v;
        }

        public static void DoLifestealDirect(this Player player, NPC target, int amount, float cooldownMult = 1f)
        {
            if (target is not null && !target.CanBeChasedBy()) return;
            amount = Math.Min(amount, player.statLifeMax2 - player.statLife);
            amount = Math.Min(amount, 20);
            if (amount <= 0 || player.lifeSteal <= 0f || player.moonLeech) return;
            player.lifeSteal -= amount * cooldownMult;
