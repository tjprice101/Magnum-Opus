...
{
    /// <summary>
    /// Self-contained GPU primitive trail renderer for Resurrection of the Moon projectiles.
    /// Adapted from the Serenade/Eternal Moon trail renderer for bullet/comet-style trails.
    /// </summary>
    [Autoload(Side = ModSide.Client)]
    public sealed class CometTrailRenderer : ModSystem
    {
        ...
        private static CometVertexType[] _vertices;
        ...
        public override void Load()
        {
            Main.QueueMainThreadAction(() =>
            {
                ...
                _vertexBuffer = new DynamicVertexBuffer(_device, CometVertexType.VertexDecl, MaxVertices, BufferUsage.WriteOnly);
                _indexBuffer = new DynamicIndexBuffer(_device, IndexElementSize.SixteenBits, MaxIndices, BufferUsage.WriteOnly);
            });

            _vertices = new CometVertexType[MaxVertices];
        ...
        /// </summary>
        public static void RenderTrail(Vector2[] positions, CometTrailSettings settings, int pointCount = 0)
        {
            ...
            // Compute arc-length-based completion ratios
        ...
        private static void BuildMesh(int count, CometTrailSettings settings, ref int vertCount, ref int idxCount)
        {
            vertCount = 0;
            idxCount = 0;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                // Top vertex
                _vertices[vertCount] = new CometVertexType(
                    pos + offset + normal * halfWidth,
                    color, new Vector2(u, 0f));

                // Bottom vertex
                _vertices[vertCount + 1] = new CometVertexType(
                    pos + offset - normal * halfWidth,
