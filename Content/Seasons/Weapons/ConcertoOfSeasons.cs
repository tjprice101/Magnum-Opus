...
namespace MagnumOpus.Content.Seasons.Weapons
{
    ...
    /// 
    /// MECHANICS:
    /// - Left Click: Seasonal Spell - Cycles through Spring → Summer → Autumn → Winter spells
    ...
    public class ConcertoOfSeasons : ModItem
    {
        ...
        public override void HoldItem(Player player)
        {
            ...
            if (player.altFunctionUse == 2 && player.channel && player.CheckMana(Item, -1, true))
            {
                ...
                // Charging VFX - all seasons converge
                if (chargeTime % 5 == 0)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        var particle = new GenericGlowParticle(particlePos, (player.Center - particlePos).SafeNormalize(Vector2.Zero) * 2f, seasonColor * 0.5f, 0.25f, 18, true);
                ...
                if (chargeTime >= 60 && chargeTime % 15 == 0 && Main.myPlayer == player.whoAmI)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        CustomParticles.GenericFlare(player.Center + burstAngle.ToRotationVector2() * 25f, burstColor, 0.55f, 18);
                    }
                    CustomParticles.HaloRing(player.Center, Color.White * 0.5f, 0.5f, 18);
            ...
            if (Main.rand.NextBool(10))
            {
                ...
                var particle = new GenericGlowParticle(particlePos, particleVel, seasonColor * 0.35f, 0.18f, 20, true);
                MagnumParticleHandler.SpawnParticle(particle);
            }
        }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            CustomParticles.HaloRing(position, seasonColor * 0.5f, 0.35f, 14);

            for (int i = 0; i < 6; i++)
            {
                Vector2 burstVel = velocity.SafeNormalize(Vector2.Zero).RotatedByRandom(0.8f) * Main.rand.NextFloat(3f, 6f);
                var burst = new GenericGlowParticle(position, burstVel, seasonColor * 0.5f, 0.22f, 16, true);
