...
{
    /// <summary>
    /// Configuration for Symphonic Bellfire's rocket/crescendo/explosion primitive trails.
    /// </summary>
    public class RocketTrailSettings
    {
        ...
        public float GetWidth(float t) => WidthFunc != null ? WidthFunc(t) : Width * (1f - t * 0.5f);
        public Color GetColor(float t) => ColorFunc != null ? ColorFunc(t) : Color.Lerp(ColorStart, ColorEnd, t);
    }

    /// <summary>
    /// GPU primitive trail renderer for Symphonic Bellfire Annihilator weapons.
    /// Builds triangle-strip ribbon mesh and renders with custom pixel shaders.
    /// </summary>
    public class SymphonicBellfirePrimitiveRenderer : IDisposable
    {
        private DynamicVertexBuffer vertexBuffer;
        private DynamicIndexBuffer indexBuffer;
        private SymphonicBellfireVertexType[] vertices;
        private short[] indices;
        private bool disposed;
        private const int MaxVertices = 2048;
        private const int MaxIndices = 6144;

        public SymphonicBellfirePrimitiveRenderer()
        {
            vertices = new SymphonicBellfireVertexType[MaxVertices];
        ...
        private void EnsureBuffers(GraphicsDevice device)
        {
            if (vertexBuffer == null || vertexBuffer.IsDisposed)
                vertexBuffer = new DynamicVertexBuffer(device, SymphonicBellfireVertexType._vertexDeclaration, MaxVertices, BufferUsage.WriteOnly);
        ...
        public void DrawTrail(SpriteBatch sb, List<Vector2> points, RocketTrailSettings settings, Vector2 screenOffset)
        {
            ...
            for (int i = 0; i < finalPositions.Length; i++)
            {
                ...
                vertices[vi] = new SymphonicBellfireVertexType(screenPos + normal * halfWidth, color, new Vector3(t, 0f, widthCorrection));
                vertices[vi + 1] = new SymphonicBellfireVertexType(screenPos - normal * halfWidth, color, new Vector3(t, 1f, widthCorrection));
                vertexCount += 2;
            ...
            if (settings.BloomIntensity > 0f && validPoints.Count > 0)
            {
                try
                {
                    ...
                    if (bloomTex != null)
                    {
                        ...
                        float bloomScale = settings.GetWidth(0f) * 0.02f;
                        sb.Draw(bloomTex, validPoints[0] - screenOffset, null, bloomCol, 0f,
                            bloomTex.Size() / 2f, bloomScale, SpriteEffects.None, 0f);

                        // Rocket exhaust secondary bloom
                        if (validPoints.Count > 2)
                        {
                            Color exhaustBloom = Color.Lerp(settings.ColorStart, new Color(255, 100, 30), 0.4f) * (settings.BloomIntensity * 0.4f);
                            sb.Draw(bloomTex, validPoints[1] - screenOffset, null, exhaustBloom, 0f,
                                bloomTex.Size() / 2f, bloomScale * 0.7f, SpriteEffects.None, 0f);
                        }
