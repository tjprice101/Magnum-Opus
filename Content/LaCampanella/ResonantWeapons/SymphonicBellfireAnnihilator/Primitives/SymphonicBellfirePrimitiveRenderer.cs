...
namespace MagnumOpus.Content.LaCampanella.ResonantWeapons.SymphonicBellfireAnnihilator.Primitives
{
    ...
    public class SymphonicBellfirePrimitiveRenderer : IDisposable
    {
        ...
        public void DrawTrail(SpriteBatch sb, List<Vector2> points, RocketTrailSettings settings, Vector2 screenPos)
        {
            ...
            for (int i = 0; i <= segCount; i++)
            {
                ...
                float width = settings.Width * (1f - t * 0.5f);
                Color color = Color.Lerp(settings.ColorStart, settings.ColorEnd, t);

                vertices[i * 2] = new SymphonicBellfireVertexType(pos + perp * width, color, new Vector3(t, 0f, 1f));
                vertices[i * 2 + 1] = new SymphonicBellfireVertexType(pos - perp * width, color, new Vector3(t, 1f, 1f));
            }
