...
{
    /// <summary>
    /// Configuration for a Grandiose Chime primitive trail.
    /// Supports both delegate-based width/color and simple start/end values.
    /// </summary>
    public class GrandioseBeamTrailSettings
    {
        ...
        /// <summary>Custom shader to use for rendering. If null, falls back to BasicEffect.</summary>
        ...
        public float GetWidth(float t) => WidthFunc != null ? WidthFunc(t) : Width * (1f - t * 0.6f);
        public Color GetColor(float t) => ColorFunc != null ? ColorFunc(t) : Color.Lerp(ColorStart, ColorEnd, t);
    }

    /// <summary>
    /// GPU primitive trail renderer for Grandiose Chime weapons.
    ...
    public class GrandioseChimePrimitiveRenderer : IDisposable
    {
        ...
        public void DrawTrail(SpriteBatch sb, List<Vector2> points, GrandioseBeamTrailSettings settings, Vector2 screenOffset)
        {
            ...
            // Optionally smooth positions via Catmull-Rom
            ...
            for (int i = 0; i < finalPositions.Length; i++)
            {
                ...
                int vi = vertexCount;
                vertices[vi] = new GrandioseChimeVertexType(left, color, new Vector3(t, 0f, widthCorrection));
                vertices[vi + 1] = new GrandioseChimeVertexType(right, color, new Vector3(t, 1f, widthCorrection));
                vertexCount += 2;
            ...
            try
            {
                ...
                try
                {
                    trailTex = ModContent.Request<Texture2D>(settings.TrailTexturePath, AssetRequestMode.ImmediateLoad)?.Value;
                }
                catch { }
                ...
                // Apply shader or fallback to BasicEffect
            ...
            if (settings.BloomIntensity > 0f && validPoints.Count > 0)
            {
                try
                {
                    ...
                    if (bloomTex != null)
                    {
                        ...
                        // Mid-trail glow for depth
                        if (validPoints.Count > 3)
                        {
                            int midIdx = validPoints.Count / 2;
                            Color midBloom = Color.Lerp(settings.ColorStart, settings.ColorEnd, 0.5f) * (settings.BloomIntensity * 0.5f);
                            sb.Draw(bloomTex, validPoints[midIdx] - screenOffset, null, midBloom, 0f,
