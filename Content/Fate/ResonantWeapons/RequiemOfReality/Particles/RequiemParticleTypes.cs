...
{
    // ========================================================================================
    // REQUIEM PARTICLE TYPES — 6 unique particle types for Requiem of Reality
    ...
    /// </summary>
    public class RequiemMoteParticle : RequiemParticle
    {
        private static Asset<Texture2D> _tex;

        public RequiemMoteParticle(Vector2 pos, Vector2 vel, Color color, float scale, int lifetime)
        {
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            sb.Draw(_tex.Value, drawPos, null, RequiemUtils.Additive(DrawColor, alpha * 0.7f),
    ...
    /// based on velocity. Used for impacts and swing accents.
    /// </summary>
    public class RequiemSparkParticle : RequiemParticle
    {
        private static Asset<Texture2D> _tex;

        public RequiemSparkParticle(Vector2 pos, Vector2 vel, Color color, float scale, int lifetime)
        {
            Position = pos; Velocity = vel; DrawColor = color; Scale = scale;
            Lifetime = lifetime; UseAdditiveBlend = true;
        }
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            Vector2 drawPos = Position - Main.screenPosition;
            sb.Draw(_tex.Value, drawPos, null, RequiemUtils.Additive(DrawColor, alpha),
    ...
    /// in alpha blend mode. The musical heartbeat of Requiem.
    /// </summary>
    public class RequiemNoteParticle : RequiemParticle
    {
        private static Asset<Texture2D>[] _noteTex;
        private readonly int _noteVariant;

        public RequiemNoteParticle(Vector2 pos, Vector2 vel, Color color, float scale, int lifetime)
        {
            Position = pos; Velocity = vel; DrawColor = color; Scale = scale;
    ...
    /// Used for combo triggers and spectral blade spawns.
    /// </summary>
    public class RequiemGlyphParticle : RequiemParticle
    {
        private static Asset<Texture2D>[] _glyphTex;
        private readonly int _glyphVariant;

        public RequiemGlyphParticle(Vector2 pos, Color color, float scale, int lifetime)
        {
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            float growScale = Scale * (0.6f + RequiemUtils.SineOut(MathHelper.Clamp(t * 3f, 0f, 1f)) * 0.4f);
            Vector2 drawPos = Position - Main.screenPosition;
            sb.Draw(tex, drawPos, null, RequiemUtils.Additive(DrawColor, alpha * 0.6f),
    ...
    /// Used for impacts and combo explosions.
    /// </summary>
    public class RequiemBloomFlare : RequiemParticle
    {
        private static Asset<Texture2D> _tex;

        public RequiemBloomFlare(Vector2 pos, Color color, float scale, int lifetime)
        {
            Position = pos; Velocity = Vector2.Zero; DrawColor = color; Scale = scale;
            Lifetime = lifetime; UseAdditiveBlend = true;
        }
        ...
        public override void Draw(SpriteBatch sb)
        {
            _tex ??= ModContent.Request<Texture2D>("MagnumOpus/Assets/SandboxLastPrism/Orbs/SoftGlow");
            float t = LifetimeCompletion;
            float expand = RequiemUtils.ExpOut(t);
            float alpha = 1f - t * t;
            float drawScale = Scale * (0.5f + expand * 1.5f);
            Vector2 drawPos = Position - Main.screenPosition;
            sb.Draw(_tex.Value, drawPos, null, RequiemUtils.Additive(DrawColor, alpha * 0.8f),
    ...
    /// Used for ambient atmospheric effects around the weapon.
    /// </summary>
    public class RequiemNebulaWisp : RequiemParticle
    {
        private static Asset<Texture2D> _tex;

        public RequiemNebulaWisp(Vector2 pos, Vector2 vel, Color color, float scale, int lifetime)
        {
            Position = pos; Velocity = vel; DrawColor = color; Scale = scale;
            Lifetime = lifetime; UseAdditiveBlend = true;
            Rotation = Main.rand.NextFloat(MathHelper.TwoPi);
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            Vector2 drawPos = Position - Main.screenPosition;
            sb.Draw(_tex.Value, drawPos, null, RequiemUtils.Additive(DrawColor, alpha * 0.35f),
                Rotation, _tex.Value.Size() / 2f, Scale, SpriteEffects.None, 0f);
        }
    }
}
