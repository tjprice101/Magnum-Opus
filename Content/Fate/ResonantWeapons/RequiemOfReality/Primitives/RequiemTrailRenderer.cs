...
{
    /// <summary>
    /// GPU primitive trail renderer for Requiem of Reality.
    /// Self-contained — builds indexed triangle strips from point arrays
    /// with CatmullRom smoothing, parallel-transport normals, and
    /// orthographic projection. Inspired by EternalMoon's LunarTrailRenderer.
    /// </summary>
    ...
    public class RequiemTrailRenderer : ModSystem
    {
        ...
        public static void RenderTrail(Vector2[] points, RequiemTrailSettings settings, int pointCount, int smoothing = 2)
        {
            ...
            var vertices = new RequiemVertexType[vertexCount];
            ...
            for (int i = 0; i < smoothedCount; i++)
            {
                ...
                vertices[i * 2] = new RequiemVertexType(
                    pos + normal * width * 0.5f,
                    color,
                    new Vector3(progress, 0f, width));

                vertices[i * 2 + 1] = new RequiemVertexType(
                    pos - normal * width * 0.5f,
                    color,
