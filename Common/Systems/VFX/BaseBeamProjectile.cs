...
namespace MagnumOpus.Common.Systems.VFX
{
    ...
    ///     {
    ///         // Aim at target NPC or fixed distance
    ///         return Projectile.Center + Projectile.velocity.SafeNormalize(Vector2.UnitX) * 1200f;
    ///     }
    /// }
    /// </code>
    /// </summary>
            ...
            try
            {
                ...
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    vertices[i * 2] = new VertexPositionColorTexture(
                        new Vector3(screenPos + perp * width * 0.5f, 0), color, new Vector2(u, 0));
                    vertices[i * 2 + 1] = new VertexPositionColorTexture(
                        new Vector3(screenPos - perp * width * 0.5f, 0), color, new Vector2(u, 1));
                }
                ...
                for (int i = 0; i < points.Length - 1; i++)
                {
                    int b = i * 2;
                    indices[idx++] = (short)b;
                    indices[idx++] = (short)(b + 1);
                    indices[idx++] = (short)(b + 2);
                    indices[idx++] = (short)(b + 1);
                    indices[idx++] = (short)(b + 3);
                    indices[idx++] = (short)(b + 2);
                }
                
                DrawPrimitives(vertices, indices, vertexCount, triangleCount, additive);
            }
            finally
            {
