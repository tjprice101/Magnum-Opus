...
namespace MagnumOpus.Common.Systems.VFX
{
    ...
    /// 
    /// OPTIMIZED: Uses static pooled buffers to eliminate per-frame allocations.
    /// 
    ...
    public class BeamVertexStrip
    {
        ...
        // Static buffers shared by all BeamVertexStrip instances to prevent per-frame allocations
        private const int PooledVertexCapacity = 256; // Enough for 128 points
        ...
        private static BasicEffect _pooledEffect;
        private static bool _poolInitialized = false;
        
        /// <summary>
        /// Initialize pooled buffers once on first use (lazy initialization).
        /// </summary>
        private static void EnsurePoolInitialized()
        {
            ...
            _pooledEffect = new BasicEffect(Main.instance.GraphicsDevice)
            {
            ...
            float uvScroll, float widthMult, float opacityMult, bool forceWhite)
        {
            ...
            try
            {
                ...
                if (_pooledVertices == null || _pooledEffect == null) return;
                ...
                {
                    int baseV = i * 2;
                    _pooledIndices[idx++] = (short)baseV;
                    _pooledIndices[idx++] = (short)(baseV + 1);
                    _pooledIndices[idx++] = (short)(baseV + 2);
                    _pooledIndices[idx++] = (short)(baseV + 1);
                    _pooledIndices[idx++] = (short)(baseV + 3);
                    _pooledIndices[idx++] = (short)(baseV + 2);
                }
                
                // Draw using pooled effect
                ...
                // Reuse pooled effect - just update matrices
                _pooledEffect.View = Matrix.CreateLookAt(Vector3.Backward, Vector3.Zero, Vector3.Up);
                _pooledEffect.Projection = Matrix.CreateOrthographicOffCenter(0, Main.screenWidth, Main.screenHeight, 0, -1, 1);
                _pooledEffect.World = Matrix.Identity;
                
                foreach (var pass in _pooledEffect.CurrentTechnique.Passes)
                {
                    pass.Apply();
                }
                
                device.DrawUserIndexedPrimitives(PrimitiveType.TriangleList,
                    _pooledVertices, 0, vertexCount, _pooledIndices, 0, triangleCount);
                
                device.BlendState = prevBlend;
                // DO NOT dispose pooled effect - it's reused!
            }
            finally
            {
