...
float4 SakuraPetalBloomPS(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float bloomRadius = lerp(0.15, 0.5, saturate(uPhase * 2.0)); // 0→0.5 phase
    float scatterExpand = saturate((uPhase - 0.5) * 2.0) * 0.4;  // 0.5→1.0 phase
    ...
    // --- Color gradient: sakura centre → gold edges ---
    ...
    if (uPhase < 0.5)
        phaseBright = 0.3 + uPhase * 1.4; // 0.3→1.0
    else
        phaseBright = 1.0 - (uPhase - 0.5) * 1.2; // 1.0→0.4
...
// Simple, soft, warm — creates the sakura "aura" around the flower.
// =============================================================================
...
technique SakuraPetalBloom
{
    pass P0
    {
        PixelShader = compile ps_2_0 SakuraPetalBloomPS();
    }
}
...
technique SakuraGlowPass
{
    pass P0
    {
        PixelShader = compile ps_2_0 SakuraGlowPS();
    }
}
