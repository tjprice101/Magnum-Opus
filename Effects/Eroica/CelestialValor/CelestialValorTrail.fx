...
// Scarlet ↁEGold gradient with pulsing core energy and sakura-soft edges.
//
// Techniques:
//   HeroicTrail   EMain projectile / swing trail
//   ValorFlare    EBright impact / bloom overlay pass
// =============================================================================
...
float4 HeroicTrailPS(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    // --- Heroic gradient: scarlet at head ↁEgold at tail ---
...
float4 ValorFlarePS(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float dist = length(centred) * 2.0; // 0ↁE from centre to edge

    // Soft radial falloff
    float radial = saturate(1.0 - dist * dist);

    // Gold core ↁEscarlet fringe gradient
